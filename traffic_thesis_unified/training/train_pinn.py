...
def main():
    ...
    lambda_physics = float(os.environ.get("LAMBDA_PHYSICS", 0.1))
    ...
    for epoch in range(num_epochs):
        ...
        phys_loss = physics_regularization(pred)
    ...
    model.load_state_dict(torch.load("checkpoints/pinn_model.pth", map_location=device))
    ...
    results.to_csv("results/pinn_results.csv", index=False)

    print("\nBest epoch:", best_epoch)
    print("Saved checkpoint: checkpoints/pinn_model.pth")
    print("Saved results: results/pinn_results.csv")
    print(results)
