...
def get_unified_data(seq_len=12, test_size=0.2, val_size=0.1, random_state=42):
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        X_seq, y_seq, test_size=test_size, shuffle=False, random_state=random_state
    )

    val_relative = val_size / (1 - test_size)

    X_train, X_val, y_train, y_val = train_test_split(
        X_train, y_train, test_size=val_relative, shuffle=False, random_state=random_state
    )
...
if __name__ == "__main__":
    X_train, y_train, X_val, y_val, X_test, y_test, meta = get_unified_data()
